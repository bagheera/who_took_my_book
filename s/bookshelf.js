...
var MyBooks = BookShelf.extend( {
	...
	render_header : function(){
		$("#my_table")
		.append(
				'<tr><th class="colone"></th><th class="coltwo">Book</th><th class="colthree">Lent to</th><th class="colfour"></th></tr>');
	},
	...
	render : function(books) {
		...
		if (books) {
			...
			$("#my_table")
					.append(
							'<tr class="nothing"><td>Nothing yet. Use the lookup box above to start adding to your collection.</td></tr>');
		}
	},
	...
	newRow : function(book) {
		...
		+ " by " + book.author + "</td><td>" + myBooks.borrower(book)
		+ "</td><td class='action'>" + myBooks.lend_link(book)
		+ "</td></tr>");	
	},
...
var BorrowedBooks = BookShelf.extend( {
	...
	render : function(books) {
		...
		if (books) {
			$("#borrowed_table")
					.append(
							'<tr><th class="colone">From</th><th class="coltwo">Book</th><th class="colthree"></th><th class="colfour"></th></tr>');
			$.each(books, this.borrowedBookrow);
		} else {
			$("#borrowed_table")
					.append(
							'<tr class="nothing"><td>Nothing yet. Don&apos;t you want to read any of the books below?</td></tr>');
		}
	},
	...
	borrowedBookrow : function (i) {
		if (borrowedBooks.not_to_be_shown(this))
			return;
		$("#borrowed_table").append(
				"<tr><td>" + this.owner + "</td><td>" + this.title + " by "
						+ this.author + "</td><td></td><td class='action'>"
						+ borrowedBooks.return_link(this) + "</td></tr>");
	}
});
/**********************************************************************************/
var OtherBooks = BookShelf.extend( {
	...
	render : function(books) {
		...
		if (books.length > 0) {
			$("#others_table")
					.append(
							'<tr><th class="colone">Owner</th><th class="coltwo">Book</th><th class="colthree">Lent to</th><th class="colfour"></th></tr>');
			$.each(books, this.othersbookrow);
		} else {
			$("#others_table").append(
					'<tr class="nothing"><td>Nothing here? That can&apos;t be true!</td></tr>');
		}
	},
	...
	othersbookrow : function(i) {
		...
		$("#others_table").append(
				"<tr><td>" + this.owner + "</td><td>" + this.title + " by "
						+ this.author + "</td><td>" + otherBooks.borrower(this)
						+ "</td><td class='action'>" + otherBooks.borrow_link(this)
						+ "</td></tr>");
	}
});
/**********************************************************************************/
...
function updateBookCount(data){
	...
	$("#book_count").append(
			"Overall summary of your books: " + own_count + " owned, "
					+ myBooks.lent_count(data.mybooks) + " lent, " + borrow_count
					+ " borrowed.");	
}
