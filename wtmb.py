...
class Book(db.Model):
  ...
  @staticmethod
  def my_books():
      return Book.gql("WHERE owner = :1", AppUser.current_AppUser())

  @staticmethod
  def borrowed_books():
      return Book.gql("WHERE borrower = :1", AppUser.current_AppUser())

  @staticmethod
  def others_books():
      return Book.gql("WHERE owner != :1 ORDER BY owner", AppUser.current_AppUser())
###################################################################
